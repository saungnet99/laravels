                    ...
                    <div class="col-12 col-md-3 border-end">
                        <div class="card-body">
                            ...
                            <div class="list-group list-group-transparent">
                                {{-- Nav links --}}
                                @include('user.pages.edit-cards.includes.nav-link', ['link' => 'testimonial'])
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-md-9 d-flex flex-column">
                        <form action="{{ route('user.update.testimonial', Request::segment(3)) }}" method="post" enctype="multipart/form-data" id="myForm">
                            @csrf
                            <div class="card-body">
                                <h3 class="card-title mb-4">{{ __('Testimonials') }}</h3>
                                ...
                                <div class="row">
                                    ...
                                    <div class="row" id="{{ $i }}">
                                        <div class="col-md-6 col-xl-6">
                                            <div class="mb-3">
                                                <label class="form-label required">{{ __('Reviewer Image') }} <span
                                                        class='text-muted'>({{ __('Recommended : 200 x 200 pixels')
                                                        }})</span></label>
                                                <div class="input-group mb-2">
                                                    <input type="text"
                                                        class="image{{ $testimonials[$i]->id }} media-model form-control"
                                                        name="reviewer_image[]"
                                                        placeholder="{{ __('Reviewer Image') }}"
                                                        value="{{ $testimonials[$i]->reviewer_image }}" required>
                                        ...
                                        <div class="col-md-6 col-xl-6">
                                            <div class="mb-3">
                                                <label class="form-label required">{{ __('Reviewer Name') }}</label>
                                                <input type="text" class="form-control" name="reviewer_name[]"
                                                    placeholder="{{ __('Reviewer Name') }}"
                                                    maxlength="191"
                                                    value="{{ $testimonials[$i]->reviewer_name }}" required>
                                            </div>
                                        </div>

                                        <div class="col-md-6 col-xl-6">
                                            <div class="mb-3">
                                                <label class="form-label required">{{ __('Reviewer Subtext') }}</label>
                                                <input type="text" class="form-control" name="review_subtext[]"
                                                    placeholder="{{ __('Reviewer Subtext') }}"
                                                    value="{{ $testimonials[$i]->review_subtext }}" required>
                                            </div>
                                        </div>

                                        <div class="col-md-6 col-xl-6">
                                            <div class="mb-3">
                                                <label class="form-label required">{{ __('Review') }}</label>
                                    ...
                                    <div id="more-testimonial" class="row"></div>

                                    {{-- Add button --}}
                                    <div class="col-lg-12 mb-5">
                                        <button type="button" onclick="addTestimonial()" class="btn btn-primary">
                                            {{ __('Add New') }}
                                        </button>
                                    </div>
                                </div>
...
<script>
    ...
    function addTestimonial() {
        "use strict";
        if (count>={{ $plan_details->no_testimonials }}) {
            new swal({
                title: `{{ __('Oops!') }}`,
                icon: 'warning',
