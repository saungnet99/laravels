                        ...
                        <div class="card">
                            <div class="card-body p-4 text-center">
                                <span class="img-rounded">
                                    <img src="{{ asset(Auth::user()->profile_image == '' ? 'img/favicon.png' : auth()->user()->profile_image) }}" alt="{{ auth()->user()->name }}">
                                </span>
                                <h3 class="m-0 mt-2 mb-1">{{ __($account_details->name) }}</h3>
                                <div >
                                    {{ $account_details->email == '' ? __('Not Available') : $account_details->email }}
                                </div>
                                <div class="mt-3">
                                    <span
                                        class="badge bg-green text-white">{{ __($account_details->role_id == 2 ? 'Customer' : '') }}</span>
                                </div>
                            </div>
                            <div class="d-flex">
