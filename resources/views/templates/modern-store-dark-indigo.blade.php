...
<body data-bs-theme="dark" dir="{{ App::isLocale('ar') || App::isLocale('ur') || App::isLocale('he') ? 'rtl' : 'ltr' }}">
    <!-- Preloader -->
            ...
            <div class="container-xl">
                ...
                <div class="navbar-nav flex-row order-md-last">
                    ...
                    <div class="nav-item dropdown">
                        <div class="nav-item">
                            <a class="position-relative cursor-pointer text-white" data-bs-toggle="modal"
                                data-bs-target="#cartItems">
                                <i class="ti ti-shopping-bag"></i>
                                <span class="badge bg-indigo text-indigo-fg badge-notification badge-pill"
                                    id="badge">0</span></a>
                        </div>
                    </div>
                </div>
                ...
                <div class="container-xl">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 pt-6 px-1">
                            <h3 class="alert alert-important alert-indigo">{{ $business_card_details->sub_title }}</h3>
                        </div>
                                    ...
                                    @foreach ($products as $product)
                                        <div class="col-sm-6 col-lg-3 col-6 py-2">
                                            <div class="card custom-card-style">
                                                ...
                                                <div class="card-body">
                                                    <span class="badge bg-indigo text-white"
                                                        id="{{ $product->product_id }}_subtitle">{{ $product->category_name }}</span>
                                                    <span
                                                        class="badge bg-indigo text-white">{{ $product->badge }}</span>
                                                    ...
                                                    <p class="text-white mb-3">{{ $product->product_subtitle }}
                                                    </p>
                            ...
                            <ul class="list-inline list-inline-dots mb-0">
                                <li class="list-inline-item"><a href="{{ $shareComponent['facebook'] }}"
                                        target="_blank" class="link-light"><i
                                            class="ti ti-brand-facebook-filled text-indigo"></i></a></li>
                                <li class="list-inline-item"><a href="{{ $shareComponent['twitter'] }}"
                                        class="link-light"><i class="ti ti-brand-twitter-filled text-indigo"></i></a>
                                </li>
                                <li class="list-inline-item"><a href="{{ $shareComponent['linkedin'] }}"
                                        target="_blank" class="link-light"><i
                                            class="ti ti-brand-linkedin text-indigo"></i></a></li>
                                <li class="list-inline-item"><a href="{{ $shareComponent['telegram'] }}"
                                        target="_blank" class="link-light"><i
                                            class="ti ti-brand-telegram text-indigo"></i></a></li>
                                <li class="list-inline-item"><a href="{{ $shareComponent['whatsapp'] }}"
                                        target="_blank" class="link-light"><i
                                            class="ti ti-brand-whatsapp text-indigo"></i></a></li>
                            </ul>
                        </div>
                            ...
                            <ul class="list-inline list-inline-dots mb-0">
                                @if ($plan_details['hide_branding'] == 1)
                                    <li class="list-inline-item">
                                        {{ __('Copyright') }} &copy; <span id="year"></span> <a
                                            ...
                                            class="link-light text-indigo"><strong>{{ $card_details->title }}</strong></a>.
                                ...
                                @else
                                    <li class="list-inline-item">
                                        {{ __('Copyright') }} &copy; <span id="year"></span> <a
                                            href="{{ url($business_card_details->card_url) }}"
                                            class="link-light text-indigo"><strong>{{ config('app.name') }}</strong></a>.
                                        {{ __('All rights reserved.') }}
                                    </li>
                                @endif
                            </ul>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <a class="btn btn-indigo" id="place-order"
                    ...
                    <div id="empty-cart" class="pt-6 p-3">
                        ...
                        <a class="btn btn-indigo d-flex" data-bs-dismiss="modal"
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-indigo" data-bs-dismiss="modal">{{ __('Close') }}</button>
    ...
    <script>
        ...
        function updateList() {
            ...
            cart.forEach((item, index) => {
                ...
                cart_items += `<h4 class="text-white mb-1">${item.product_name}</h4>`;
                cart_items += `<small class="text-white">${item.subtitle}</small>`;
                cart_items +=
                    `<p class="text-white mt-1"><strong>${currency} ${total_price}</strong></p>`;
                cart_items += `</div>`;
                cart_items += `<div class="card-footer">`;
                cart_items += `<div class="d-flex flex-wrap justify-content-between">`;
                cart_items +=
                    `<a onclick="reduceQty(${index})" class="btn btn-indigo flex-fill mb-1 ${window.innerWidth <= 768 ? 'btn-sm' : 'mx-2'}">-</a>`;
                cart_items +=
                    `<a class="btn btn-white flex-fill mb-1 ${window.innerWidth <= 768 ? 'btn-sm' : 'mx-2'}">${item.qty}</a>`;
                cart_items +=
                    `<a onclick="addQty(${index})" class="btn btn-indigo flex-fill mb-1 ${window.innerWidth <= 768 ? 'btn-sm' : 'mx-2'}">+</a>`;
                cart_items +=
                    `<a onclick="removeFromCart(${index})" class="btn btn-indigo flex-fill mb-1 ${window.innerWidth <= 768 ? 'btn-sm' : 'mx-2'}">x</a>`;
                cart_items += `</div>`;
                cart_items += `</div>`;
                cart_items += `</div>`;
                cart_items += `</div>`;
            ...
            if (grandTotal > 0) {
                cart_items +=
                    `<br><h3 class="pl-4 pt-4 pr-4 font-bold text-white">{{ __('Grand total:') }} ${currency} ${grandTotal}</h3>`;
            }
        ...
        function updateBadge() {
            ...
            if (badgeCount > 0) {
                ...
                $("#place-order").show().attr("class", "btn btn-indigo d-flex");
            } else {
                $("#place-order").hide().attr("class", "btn btn-indigo d-none");
                $("#badge").hide();
                $("#empty-cart").show();
            }
        }
