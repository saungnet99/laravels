...
<body class="antialiased lg:bg-yellow-800 bg-grey-lighter text-body font-body"
        ...
        @if ($business_card_details->password == null || Session::get('password_protected') == true)
            @if ($business_card_details != null)
                <div id="profile" class="leading-tight min-h-screen p-1">
                    ...
                    <div
                        class="max-w-xl w-full mx-auto bg-white rounded-lg overflow-hidden shadow-lg relative {{ $business_card_details->cover_type == 'photo' ? '' : 'pt-24' }}">
                        ...
                        <div class="border-b px-4 pb-2">
                            <div class="text-center -mt-16 sm:text-left sm:flex mb-4">
                                <img class="h-32 w-32 rounded-lg border-2 border-white mr-4 md:ml-24 lg:ml-12 profile"
                            ...
                            <div class="text-center">
                                <div class="pt-0 pb-3">
                                    ...
                                    @if ($business_card_details->description != null || $business_card_details->address != null)
                                        <div class="pt-4 text-left text-gray-800">
                        ...
                        @foreach ($feature_details as $feature)
                            <ul class="grid grid-flow-col grid-cols-1 grid-rows-1 mb-1 mb-5">
                                ...
                                @if ($feature->type != 'iframe' && $feature->type != 'youtube' && $feature->type != 'map' && $feature->type != 'text')
                                    <li>
                                        <div class="flex items-center w-full px-5 ">
                                            <div
                                                class="flex justify-center items-center content-center bg-gradient-to-br from-yellow-300 to-yellow-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
                        ...
                        @if ($customTexts != null && !$customTexts->isEmpty())
                            @foreach ($customTexts as $customText)
                                <div class="w-full px-5 align-middle py-4 border-t border-b">
                                    <p class="font-semibold text-lg">{{ __($customText->label) }}</p>
                                </div>

                                <div class="px-5 py-4">
                                    <div class="w-full my-6">
                                        <p class="font-semibold break-word text-lg">
                                                    ...
                                                    @if (($whatsAppNumberExists == true && $whatsAppNumberExists == true) && $service_detail->enable_enquiry == 'Enabled')
                                                        <div class="mt-5 mb-2">
                                                            <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product/service:') }} {{ $service_detail->service_name }}. {{ __('Please provide more details.') }}"
                                                                rel="noopener nofollow noreferrer" target="_blank"
                                                                class="flex-1 rounded-lg bg-yellow-500 font-semibold hover:bg-yellow-800 text-white antialiased px-4 py-2">
                        ...
                        @if ($product_details != null && !$product_details->isEmpty())
                            <div class="w-full px-5 align-middle py-4 border-b">
                                <p class="text-dark font-semibold text-lg">{{ __('Products') }}</p>
                            </div>
                                            ...
                                            <div class="w-full mb-1 mt-1 justify-between items-center">
                                                <div>
                                                    ...
                                                    <span id="{{ $product_detail->product_id }}_subtitle"
                                                        class="text-xs text-dark">{{ $product_detail->product_subtitle }}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div class="w-full overflow-hidden rounded-lg shadow-lg">
                                            <a href="{{ url($galleries_detail->gallery_image) }}"
                                                ...
                                                <img class="w-full" src="{{ url($galleries_detail->gallery_image) }}"
                                                    alt="{{ $galleries_detail->caption }}" style="width: 100%; height: 350px; object-fit: cover;" />
                                            </a>
                                            <div class="px-5 py-3">
                                                <div class="mb-2">
                        ...
                        @if ($testimonials != null && !$testimonials->isEmpty())
                            <div class="w-full align-middle p-6 border-t border-b">
                                <p class="font-semibold text-lg">{{ __('Testimonials') }}</p>
                            </div>
                                ...
                                @foreach ($feature_details as $feature)
                                    @if ($feature->type == 'youtube')
                                        <div class="w-full px-5 align-middle py-4">
                                            <p class="text-gray-700 font-semibold text-lg">{{ __('Videos') }}</p>
                                        </div>
                                        <div class="mb-3">
                                            <div class="w-full overflow-hidden rounded-lg shadow-lg">
                        ...
                        @if ($iframes != null && !$iframes->isEmpty())
                            @foreach ($iframes as $iframe)
                                <div class="w-full px-5 align-middle py-4 border-b">
                                    <p class="font-semibold text-lg">{{ __($iframe->label) }}</p>
                                </div>
                                            ...
                                            @if ($payment->type != 'upi')
                                                <div class="w-3/4 mx-5 my-6">
                                                    ...
                                                    <p class="font-medium text-gray-800 pt-1 break-word text-base">
                                                        <pre class="whitespace-break-spaces text-gray-800 text-sm">{!! $payment->content !!}</pre>
                                                    </p>
                                                </div>
                                            @endif
                                            @if ($payment->type == 'upi')
                                                <div class="w-3/4 mx-5 my-6">
                                                    <p class="font-semibold break-word text-gray-800 text-lg mb-2">
                                                        {{ $payment->label }}</p>
                        ...
                        @if ($plan_details['business_hours'] == 1)
                            @if ($business_hours != null && $business_hours->is_display != 0)
                                ...
                                <div class="px-5 py-4">
                                    ...
                                    @else
                                        <div>
                                            <p class="pt-2 py-4 font-semibold text-yellow-800">{{ __('Always Open') }}
                                            </p>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <section class="py-5 lg:py-8 border-b">
                                    <div class="container mx-auto">
                                        <div class="max-w-xl mx-auto">
                                            <div class="">
                                                <div class="mb-6">
                                                    <h3 class="p-4 text-2xl font-bold capitalize border-b">
                                                        {{ __('Contact us') }}
                                                    </h3>
                                                </div>
                                                @if (Session::has('message'))
                                                    <div class="px-4 bg-gradient-to-br from-yellow-300 to-yellow-800 border-t-4 border-yellow-500 rounded-b text-yellow-900 px-4 py-3 shadow-md mb-3"
                                                        role="alert">
                                                        <div class="flex">
                                                            <div class="py-1"><svg
                                                ...
                                                <form class="px-4" action="{{ route('sent.enquiry') }}"
                                                    ...
                                                    <div class="text-center">
                                                        <button
                                                            class="mb-2 w-full py-4 bg-gradient-to-br from-yellow-300 to-yellow-800 text-sm rounded-full font-bold text-gray-50 transition duration-200">
                                                            {{ __('Send') }}
                                                        </button>
                                                    </div>
                                                </form>
                        ...
                        @if ($feature_details != null && !$feature_details->isEmpty())
                            <div id="youtube" class="px-5 py-4">
                                @foreach ($feature_details as $feature)
                                    @if ($feature->type == 'map')
                                        <div class="w-full px-5 align-middle py-4 border-b">
                                            <p class="text-gray-700 font-semibold text-lg">{{ __('Location') }}</p>
                                        </div>
                                        <div class="mb-3">
                                            <div class="w-full overflow-hidden rounded-lg shadow-lg">
                        ...
                        <div class="lg:block hidden w-full pl-6 pb-3 mx-auto pt-3 border-b">
                            <ul class="grid grid-flow-col grid-cols-6 grid-rows-1">
                                ...
                                <li class="flex send-modal-open cursor-pointer items-center">
                                    <div
                                        class="flex justify-center items-center content-center bg-gradient-to-br from-yellow-300 to-yellow-800 shadow-md hover:shadow-lg h-16 w-16 rounded-lg fill-current text-white">
                                ...
                                <li class="flex qr-modal-open cursor-pointer items-center">
                                    <div
                                        class="flex justify-center items-center content-center bg-gradient-to-br from-yellow-300 to-yellow-800 shadow-lg hover:shadow-lg h-16 w-16 rounded-lg fill-current text-white">
                        ...
                        <div id="share" class="w-full border-t border-b px-5 align-middle py-4">
                        ...
                        @if ($plan_details['hide_branding'] == 1)
                            <div class="pb-1">
                                <div
                                    ...
                                    <div class="mt-2">
                                        {{ __('Copyright') }} &copy;
                                        <a class="text-yellow-800" href="{{ url($card_details->card_url) }}">
                        ...
                        @else
                            <div class="pb-1">
                                <div
                                    ...
                                    <div class="mt-2">
                                        {{ __('Made with') }}
                                        <a class="text-yellow-800" href="{{ env('APP_URL') }}">
                                            {{ config('app.name') }} </a>
                                        <span id="year"></span>{{ __('. All Rights Reserved.') }}
                                    </div>
                                </div>
                ...
                {{-- Save Contact --}}
                <div class="lg:hidden bg-yellow-800 fixed bottom-0 w-full border-t border-gray-200 flex z-10">
                    {{-- Scan QR --}}
                    <a href="#"
                        class="flex qr-modal-open flex-grow items-center justify-center p-2 text-gray-50 hover:text-gray-50 bg-yellow-800">
                        <div class="text-center">
                    ...
                    {{-- Send vCard --}}
                    <a href="#"
                        class="flex send-modal-open flex-grow items-center justify-center p-2 text-gray-50 hover:text-gray-50 bg-yellow-800">
                        <div class="text-center">
                            <span class="block h-8 grid justify-items-center text-3xl leading-8">
                    ...
                    {{-- Download vCard --}}
                    <a href="{{ route('download.vCard', $business_card_details->card_id) }}"
                        class="flex flex-grow items-center justify-center p-2 text-gray-50 hover:text-gray-50 bg-yellow-800">
                        <div class="text-center">
                            <span class="block h-8 grid justify-items-center text-3xl leading-8">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div
                        ...
                        <div class="modal-content py-4 text-left px-6">
                            <div class="justify-between items-center">
                                ...
                                <button
                                    class="flex justify-center items-center content-center bg-gradient-to-br from-yellow-300 to-yellow-800 shadow-md hover:shadow-lg h-14 w-12 py-2 px-8 fill-current text-white"
                                    onclick="sendVcard()">
                                    {{ __('Send') }}
                                </button>
                            </div>
                    ...
                    <div
                        ...
                        <div class="modal-content py-4 text-left px-6">
                            ...
                            <a id="download"
                                ...
                                class="cursor-pointer flex justify-center items-center content-center bg-gradient-to-br from-yellow-300 to-yellow-800 shadow-lg hover:shadow-lg h-16 w-16 rounded-lg fill-current text-white qr-code-download">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
    ...
    @if ($business_card_details->password != null && Session::get('password_protected') == false)
        <div class="bg-yellow-50 p-4 flex items-center justify-center h-screen">
                            ...
                            <form action="{{ route('check.pwd', $business_card_details->card_id) }}" method="post">
                                ...
                                <div class="flex">
                                    <input type="password" name="password"
                                        class="rounded rounded-r-lg bg-gray-50 border text-gray-800 focus:ring-yellow-100 focus:border-yellow-100 block flex-1 min-w-0 w-full text-sm border-gray-100 p-2.5"
                                ...
                                <div class="flex flex-col space-y-4 mt-3">
                                    <button type="submit"
                                        class="bg-yellow-500 text-white px-4 py-2 mt-2 rounded-lg hover:bg-yellow-600 transition duration-300">{{ __('Password') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
