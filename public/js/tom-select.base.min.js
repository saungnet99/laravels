...
*/
!function (t, e) { "object" == typeof exports && "undefined" != typeof module ? module.exports = e() : "function" == typeof define && define.amd ? define(e) : (t = "undefined" != typeof globalThis ? globalThis : t || self).TomSelect = e() }(this, (function () {
    "use strict"
    function t(t, e) { t.split(/\s+/).forEach((t => { e(t) })) } class e {
        constructor() { this._events = void 0, this._events = {} } on(e, i) {
            t(e, (t => {
                const e = this._events[t] || []
                e.push(i), this._events[t] = e
            }))
        } off(e, i) {
            var s = arguments.length
            0 !== s ? t(e, (t => {
                if (1 === s) return void delete this._events[t]
                const e = this._events[t]
                void 0 !== e && (e.splice(e.indexOf(i), 1), this._events[t] = e)
            })) : this._events = {}
        } trigger(e, ...i) {
            var s = this
            t(e, (t => {
                const e = s._events[t]
                void 0 !== e && e.forEach((t => { t.apply(s, i) }))
            }))
        }
    } const i = t => (t = t.filter(Boolean)).length < 2 ? t[0] || "" : 1 == l(t) ? "[" + t.join("") + "]" : "(?:" + t.join("|") + ")", s = t => {
        if (!o(t)) return t.join("")
        let e = "", i = 0
        const s = () => { i > 1 && (e += "{" + i + "}") }
        return t.forEach(((n, o) => { n !== t[o - 1] ? (s(), e += n, i = 1) : i++ })), s(), e
    }, n = t => {
        let e = c(t)
        return i(e)
    }, o = t => new Set(t).size !== t.length, r = t => (t + "").replace(/([\$\(\)\*\+\.\?\[\]\^\{\|\}\\])/gu, "\\$1"), l = t => t.reduce(((t, e) => Math.max(t, a(e))), 0), a = t => c(t).length, c = t => Array.from(t), d = t => {
        if (1 === t.length) return [[t]]
        let e = []
        const i = t.substring(1)
        return d(i).forEach((function (i) {
            let s = i.slice(0)
            s[0] = t.charAt(0) + s[0], e.push(s), s = i.slice(0), s.unshift(t.charAt(0)), e.push(s)
        })), e
    }, u = [[0, 65535]]
    let p, h
    ...
    for (let t in f) {
        ...
        for (let i = 0; i < e.length; i++) {
            let s = e.substring(i, i + 1)
            g[s] = t
        }
    } const v = new RegExp(Object.keys(g).join("|") + "|[̀-ͯ·ʾʼ]", "gu"), m = (t, e = "NFKD") => t.normalize(e), y = t => c(t).reduce(((t, e) => t + O(e)), ""), O = t => (t = m(t).toLowerCase().replace(v, (t => g[t] || "")), m(t, "NFC"))
    const b = t => {
        const e = {}, i = (t, i) => {
            const s = e[t] || new Set, o = new RegExp("^" + n(s) + "$", "iu")
            i.match(o) || (s.add(r(i)), e[t] = s)
        }
        for (let e of function* (t) {
            for (const [e, i] of t) for (let t = e; t <= i; t++) {
                let e = String.fromCharCode(t), i = y(e)
                i != e.toLowerCase() && (i.length > 3 || 0 != i.length && (yield { folded: i, composed: e, code_point: t }))
            }
        }(t)) i(e.folded, e.folded), i(e.folded, e.composed)
        return e
    }, w = t => {
        const e = b(t), s = {}
        let o = []
        for (let t in e) {
            let i = e[t]
            i && (s[t] = n(i)), t.length > 1 && o.push(r(t))
        } o.sort(((t, e) => e.length - t.length))
        const l = i(o)
        return h = new RegExp("^" + l, "u"), s
    }, I = (t, e = 1) => (e = Math.max(e, t.length - 1), i(d(t).map((t => ((t, e = 1) => {
        let i = 0
        return t = t.map((t => (p[t] && (i += t.length), p[t] || t))), i >= e ? s(t) : ""
    })(t, e))))), _ = (t, e = !0) => {
        ...
        for (const i of e) {
            if (i.start != t.start || i.end != t.end) continue
            if (i.substrs.join("") !== t.substrs.join("")) continue
            let e = t.parts
            const s = t => {
                for (const i of e) {
                    if (i.start === t.start && i.substr === t.substr) return !1
                    if (1 != t.length && 1 != i.length) {
                        if (t.start < i.start && t.end > i.start) return !0
    ...
    class A {
        constructor() { this.parts = [], this.substrs = [], this.start = 0, this.end = 0 } add(t) { t && (this.parts.push(t), this.substrs.push(t.substr), this.start = Math.min(t.start, this.start), this.end = Math.max(t.end, this.end)) } last() { return this.parts[this.parts.length - 1] } length() { return this.parts.length } clone(t, e) {
            let i = new A, s = JSON.parse(JSON.stringify(this.parts)), n = s.pop()
        ...
        for (let e = 0; e < t.length; e++) {
            let n = t.substring(e).match(h)
            const o = t.substring(e, e + 1), r = n ? n[0] : null
            let l = [], a = new Set
            for (const t of s) {
                const i = t.last()
                if (!i || 1 == i.length || i.end <= e) if (r) {
                    const i = r.length
                    t.add({ start: e, end: e + i, length: i, substr: r }), a.add("1")
                } else t.add({ start: e, end: e + 1, length: 1, substr: o }), a.add("2")
                else if (r) {
                    let s = t.clone(e, i)
                    const n = r.length
                    s.add({ start: e, end: e + n, length: n, substr: r }), l.push(s)
                } else a.add("3")
            } if (l.length > 0) {
                l = l.sort(((t, e) => t.length() - e.length()))
                for (let t of l) S(t, s) || s.push(t)
            } else if (e > 0 && 1 == a.size && !a.has("3")) {
                i += _(s, !1)
                let t = new A
                const e = s[0]
                e && t.add(e.last()), s = [t]
    ...
    }, F = (t, e) => { if (t) return t[e] }, x = (t, e) => { if (t) { for (var i, s = e.split("."); (i = s.shift()) && (t = t[i]);); return t } }, L = (t, e, i) => {
        var s, n
    ...
    }, P = (t, e) => "number" == typeof t && "number" == typeof e ? t > e ? 1 : t < e ? -1 : 0 : (t = y(t + "").toLowerCase()) > (e = y(e + "").toLowerCase()) ? 1 : e > t ? -1 : 0
    class T {
        constructor(t, e) { this.items = void 0, this.settings = void 0, this.items = t, this.settings = e || { diacritics: !0 } } tokenize(t, e, i) {
            if (!t || !t.length) return []
            const s = [], n = t.split(/\s+/)
            var o
            return i && (o = new RegExp("^(" + Object.keys(i).map(r).join("|") + "):(.*)$")), n.forEach((t => {
                let i, n = null, l = null
                o && (i = t.match(o)) && (n = i[1], t = i[2]), t.length > 0 && (l = this.settings.diacritics ? C(t) || null : r(t), l && e && (l = "\\b" + l)), s.push({ string: t, regex: l ? new RegExp(l, "iu") : null, field: n })
            })), s
        } getScoreFunction(t, e) {
            var i = this.prepareSearch(t, e)
            return this._getScoreFunction(i)
        } _getScoreFunction(t) {
            const e = t.tokens, i = e.length
            if (!i) return function () { return 0 }
            const s = t.options.fields, n = t.weights, o = s.length, r = t.getAttrFn
            if (!o) return function () { return 1 }
            const l = 1 === o ? function (t, e) {
                const i = s[0].field
                return L(r(e, i), t, n[i] || 1)
            } : function (t, e) {
                var i = 0
                if (t.field) {
                    const s = r(e, t.field)
                    !t.regex && s ? i += 1 / o : i += L(s, t, 1)
                } else E(n, ((s, n) => { i += L(r(e, n), t, s) }))
                return i / o
            }
            return 1 === i ? function (t) { return l(e[0], t) } : "and" === t.options.conjunction ? function (t) {
                var s, n = 0
                for (let i of e) {
                    if ((s = l(i, t)) <= 0) return 0
                    n += s
                } return n / i
            } : function (t) {
                var s = 0
                return E(e, (e => { s += l(e, t) })), s / i
            }
        } getSortFunction(t, e) {
            var i = this.prepareSearch(t, e)
            ...
            if (t.query) {
                e = !0
                for (let t of i) if ("$score" === t.field) {
                    e = !1
                    break
            ...
            return i.length ? function (t, e) {
                var s, n
                for (let o of i) { if (n = o.field, s = ("desc" === o.direction ? -1 : 1) * P(r(n, t), r(n, e))) return s } return 0
            } : null
        } prepareSearch(t, e) {
            const i = {}
            var s = Object.assign({}, e)
            if (k(s, "sort"), k(s, "sort_empty"), s.fields) {
                k(s, "fields")
                const t = []
                s.fields.forEach((e => { "string" == typeof e && (e = { field: e, weight: 1 }), t.push(e), i[e.field] = "weight" in e ? e.weight : 1 })), s.fields = t
            } return { options: s, query: t.toLowerCase().trim(), tokens: this.tokenize(t, s.respect_word_boundaries, i), total: 0, items: [], weights: i, getAttrFn: s.nesting ? x : F }
        } search(t, e) {
            var i, s, n = this
            ...
            return r && s.items.sort(r), s.total = s.items.length, "number" == typeof e.limit && (s.items = s.items.slice(0, e.limit)), s
        }
    } const V = (t, e) => {
        if (Array.isArray(t)) t.forEach(e)
        else for (var i in t) t.hasOwnProperty(i) && e(t[i], i)
    ...
    }, j = t => "string" == typeof t && t.indexOf("<") > -1, q = (t, e) => {
        var i = document.createEvent("HTMLEvents")
        i.initEvent(e, !0, !1), t.dispatchEvent(i)
    }, D = (t, e) => { Object.assign(t.style, e) }, H = (t, ...e) => { var i = R(e); (t = M(t)).map((t => { i.map((e => { t.classList.add(e) })) })) }, N = (t, ...e) => { var i = R(e); (t = M(t)).map((t => { i.map((e => { t.classList.remove(e) })) })) }, R = t => {
        var e = []
        return V(t, (t => { "string" == typeof t && (t = t.trim().split(/[\11\12\14\15\40]/)), Array.isArray(t) && (e = e.concat(t)) })), e.filter(Boolean)
    }, M = t => (Array.isArray(t) || (t = [t]), t), z = (t, e, i) => {
        if (!i || i.contains(t)) for (; t && t.matches;) {
            if (t.matches(e)) return t
            t = t.parentNode
        }
    }, B = (t, e = 0) => e > 0 ? t[t.length - 1] : t[0], K = (t, e) => {
        if (!t) return -1
        e = e || t.nodeName
        for (var i = 0; t = t.previousElementSibling;)t.matches(e) && i++
        return i
    }, Q = (t, e) => { V(e, ((e, i) => { null == e ? t.removeAttribute(i) : t.setAttribute(i, "" + e) })) }, G = (t, e) => { t.parentNode && t.parentNode.replaceChild(e, t) }, J = (t, e) => {
        if (null === e) return
        if ("string" == typeof e) {
            if (!e.length) return
            e = new RegExp(e, "i")
        } const i = t => 3 === t.nodeType ? (t => {
        ...
        })(t) : ((t => { 1 !== t.nodeType || !t.childNodes || /(script|style)/i.test(t.tagName) || "highlight" === t.className && "SPAN" === t.tagName || Array.from(t.childNodes).forEach((t => { i(t) })) })(t), 0)
        i(t)
    }, U = "undefined" != typeof navigator && /Mac/.test(navigator.userAgent) ? "metaKey" : "ctrlKey"
    var W = { options: [], optgroups: [], plugins: [], delimiter: ",", splitOn: null, persist: !0, diacritics: !0, create: null, createOnBlur: !1, createFilter: null, highlight: !0, openOnFocus: !0, shouldOpen: null, maxOptions: 200, maxItems: null, hideSelected: null, duplicates: !1, addPrecedence: !1, selectOnTab: !1, preload: null, allowEmptyOption: !1, loadThrottle: 300, loadingClass: "loading", dataAttr: null, optgroupField: "optgroup", valueField: "value", labelField: "text", disabledField: "disabled", optgroupLabelField: "label", optgroupValueField: "value", lockOptgroupOrder: !1, sortField: "$order", searchField: ["text"], searchConjunction: "and", mode: null, wrapperClass: "ts-wrapper", controlClass: "ts-control", dropdownClass: "ts-dropdown", dropdownContentClass: "ts-dropdown-content", itemClass: "item", optionClass: "option", dropdownParent: null, controlInput: '<input type="text" autocomplete="off" size="1" />', copyClassesToDropdown: !1, placeholder: null, hidePlaceholder: null, shouldLoad: function (t) { return t.length > 0 }, render: {} }
    const X = t => null == t ? null : Y(t), Y = t => "boolean" == typeof t ? t ? "1" : "0" : t + "", Z = t => (t + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"), tt = (t, e) => {
        var i
        return function (s, n) {
            var o = this
        ...
        var s, n = t.trigger, o = {}
        for (s of (t.trigger = function () {
            var i = arguments[0]
            if (-1 === e.indexOf(i)) return n.apply(t, arguments)
            o[i] = arguments
        }, i.apply(t, []), t.trigger = n, e)) s in o && n.apply(t, o[s])
    }, it = (t, e = !1) => { t && (t.preventDefault(), e && t.stopPropagation()) }, st = (t, e, i, s) => { t.addEventListener(e, i, s) }, nt = (t, e) => !!e && (!!e[t] && 1 === (e.altKey ? 1 : 0) + (e.ctrlKey ? 1 : 0) + (e.shiftKey ? 1 : 0) + (e.metaKey ? 1 : 0)), ot = (t, e) => {
        const i = t.getAttribute("id")
        return i || (t.setAttribute("id", e), e)
    }, rt = t => t.replace(/[\\"']/g, "\\$&"), lt = (t, e) => { e && t.append(e) }
    function at(t, e) {
        var i = Object.assign({}, W, e), s = i.dataAttr, n = i.labelField, o = i.valueField, r = i.disabledField, l = i.optgroupField, a = i.optgroupLabelField, c = i.optgroupValueField, d = t.tagName.toLowerCase(), u = t.getAttribute("placeholder") || t.getAttribute("data-placeholder")
        ...
        return "select" === d ? (h = y.options, g = {}, f = 1, v = t => {
            var e = Object.assign({}, t.dataset), i = s && e[s]
            return "string" == typeof i && i.length && (e = Object.assign(e, JSON.parse(i))), e
        }, m = (t, e) => {
            var s = X(t.value)
            if (null != s && (s || i.allowEmptyOption)) {
                if (g.hasOwnProperty(s)) {
                    if (e) {
                        var a = g[s][l]
                        a ? Array.isArray(a) ? a.push(e) : g[s][l] = [a, e] : g[s][l] = e
                    }
                } else {
                    var c = v(t)
                    c[n] = c[n] || t.textContent, c[o] = c[o] || s, c[r] = c[r] || t.disabled, c[l] = c[l] || e, c.$option = t, g[s] = c, h.push(c)
                } t.selected && y.items.push(s)
            }
        }, y.maxItems = t.hasAttribute("multiple") ? null : 1, V(t.children, (t => {
            var e, i, s
            "optgroup" === (p = t.tagName.toLowerCase()) ? ((s = v(e = t))[a] = s[a] || e.getAttribute("label") || "", s[c] = s[c] || f++, s[r] = s[r] || e.disabled, y.optgroups.push(s), i = s[c], V(e.children, (t => { m(t, i) }))) : "option" === p && m(t)
        }))) : (() => {
            ...
            if (e) y.options = JSON.parse(e), V(y.options, (t => { y.items.push(t[o]) }))
                ...
                V(e, (t => {
                    const e = {}
                    e[n] = t, e[o] = t, y.options.push(e)
                })), y.items = e
            }
        })(), Object.assign({}, W, y, e)
    } var ct = 0
    class dt extends (function (t) {
        return t.plugins = {}, class extends t {
            constructor(...t) { super(...t), this.plugins = { names: [], settings: {}, requested: {}, loaded: {} } } static define(e, i) { t.plugins[e] = { name: e, fn: i } } initializePlugins(t) {
                var e, i
                const s = this, n = []
            ...
            constructor(t, e) {
                ...
                } else n.controlInput ? (p = $(n.controlInput), this.focus_node = p) : (p = $("<input/>"), this.focus_node = l)
                ...
                var t = this, e = t.settings.labelField, i = t.settings.optgroupLabelField, s = {
                    ...
                    }, optgroup_header: (t, e) => '<div class="optgroup-header">' + e(t[i]) + "</div>", option: (t, i) => "<div>" + i(t[e]) + "</div>", item: (t, i) => "<div>" + i(t[e]) + "</div>", option_create: (t, e) => '<div class="create">Add <strong>' + e(t.input) + "</strong>&hellip;</div>", no_results: () => '<div class="no-results">No results found</div>', loading: () => '<div class="spinner"></div>', not_loading: () => { }, dropdown: () => "<div></div>"
                }
                ...
                if (e.is_select_tag) {
                    ...
                    function o(i, s, o) { return i || (i = $('<option value="' + Z(s) + '">' + Z(o) + "</option>")), i != n && e.input.append(i), t.push(i), (i != n || r > 0) && (i.selected = !0), i } e.input.querySelectorAll("option:checked").forEach((t => { t.selected = !1 })), 0 == e.items.length && "single" == e.settings.mode ? o(n, "", "") : e.items.forEach((n => { if (i = e.options[n], s = i[e.settings.labelField] || "", t.includes(i.$option)) { o(e.input.querySelector(`option[value="${rt(n)}"]:not(:checked)`), n, s) } else i.$option = o(i.$option, n, s) }))
